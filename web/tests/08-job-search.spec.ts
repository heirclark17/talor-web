...
test.describe('Phase 08: job search', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/job-search'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/job-search/); await expect(page.locator('body')).toBeVisible(); });
