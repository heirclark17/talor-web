...
test.describe('Phase 11: star stories', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/star-stories'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/star-stories/); await expect(page.locator('body')).toBeVisible(); });
