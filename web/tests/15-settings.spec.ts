...
test.describe('Phase 15: settings', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/settings'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/settings/); await expect(page.locator('body')).toBeVisible(); });
