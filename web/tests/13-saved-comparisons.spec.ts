...
test.describe('Phase 13: saved comparisons', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/saved-comparisons'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/saved-comparisons/); await expect(page.locator('body')).toBeVisible(); });
