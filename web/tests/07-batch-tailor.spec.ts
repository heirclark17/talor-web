...
test.describe('Phase 07: batch tailor', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/batch-tailor'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/batch-tailor/); await expect(page.locator('body')).toBeVisible(); });
