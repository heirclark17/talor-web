...
test.describe('Phase 16: public pages', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/public-pages'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\//); await expect(page.locator('body')).toBeVisible(); });
