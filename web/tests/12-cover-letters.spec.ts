...
test.describe('Phase 12: cover letters', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/cover-letters'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/cover-letters/); await expect(page.locator('body')).toBeVisible(); });
