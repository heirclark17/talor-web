...
test.describe('Phase 06: tailor', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/tailor-resume'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/tailor/); await expect(page.locator('body')).toBeVisible(); });
