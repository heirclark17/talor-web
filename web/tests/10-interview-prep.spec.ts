...
test.describe('Phase 10: interview prep', () => {
  test.beforeEach(async ({ page }) => { await bypassAuth(page); await page.goto('/interview-prep'); });
  test('should load page', async ({ page }) => { await expect(page).toHaveURL(/\/interview-prep/); await expect(page.locator('body')).toBeVisible(); });
