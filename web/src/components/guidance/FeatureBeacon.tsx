...
export default function FeatureBeacon({
  ...
  return (
    <>
      ...
      <button
        onClick={handleClick}
        className="relative inline-flex items-center gap-1.5 px-2.5 py-1 bg-gradient-to-r from-blue-500 to-purple-600 text-white text-xs font-semibold rounded-full hover:from-blue-600 hover:to-purple-700 transition-all shadow-lg shadow-blue-500/30"
        aria-label={`New feature: ${title}`}
      >
        {/* Pulsing dot */}
        <span className="relative flex h-2 w-2">
          <span className="motion-safe:animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
      ...
      {showModal && (
        ...
        >
          <div
            className="glass rounded-2xl border border-theme-subtle max-w-2xl w-full overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-5"
          ...
          >
            ...
            {screenshot && (
              <div className="bg-theme-glass-5">
                <img
                  src={screenshot}
                  alt={title}
            ...
            <div className="px-6 pb-6 flex items-center gap-3">
              ...
              <button
                onClick={handleDismiss}
                className="px-6 py-3 bg-theme-glass-10 text-theme font-medium rounded-lg hover:bg-theme-glass-20 transition-colors"
              >
                Got it
              </button>
            </div>
