 ...
 *
 * Adds "Tailor with Talor" button to Indeed job postings
 */
...
(function() {
  ...
  function extractJobDetails() {
    try {
      // Job title
      const titleElement = document.querySelector('.jobsearch-JobInfoHeader-title, h1.jobsearch-JobInfoHeader-title-container');
      const title = titleElement?.textContent?.trim() || '';

      // Company name
      const companyElement = document.querySelector('[data-testid="inlineHeader-companyName"], .jobsearch-InlineCompanyRating-companyHeader a');
      const company = companyElement?.textContent?.trim() || '';

      // Location
      const locationElement = document.querySelector('[data-testid="inlineHeader-companyLocation"], .jobsearch-JobInfoHeader-subtitle-location');
  ...
  function injectTailorButton() {
    ...
    const actionContainer = document.querySelector(
      '.jobsearch-JobMetadataHeader-metadataAction, .jobsearch-JobInfoHeader-title-container, .jobsearch-ViewJobLayout-mainContent'
    );
  ...
  function handleTailorClick(e) {
    ...
    const params = new URLSearchParams({
      ...
      source: 'chrome-extension-indeed'
    });
  ...
  function init() {
    ...
    // Watch for dynamic content changes (Indeed is a SPA)
